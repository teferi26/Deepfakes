    ...
    def _reload_head_from_db(self, force: bool = False) -> bool:
        ...
        with self._head_lock:
            try:
                ...
                # Compatibilidad: algunos scripts guardan nn.Linear directamente,
                # con claves "weight"/"bias" en lugar de "fc.weight"/"fc.bias".
                # Normalizamos al formato esperado por CLIPLinearClassifier.
                if "fc.weight" not in state_dict and "weight" in state_dict:
                    state_dict = {
                        "fc.weight": state_dict.get("weight"),
                        "fc.bias": state_dict.get("bias"),
                    }
