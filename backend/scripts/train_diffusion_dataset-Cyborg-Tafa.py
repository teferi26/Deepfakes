...
def main():
    ...
    # IMPORTANTE: NO usar ds_hemg[i] aquí porque decodifica también la imagen
    # (muy lento y puede disparar errores de archivos abiertos). Leer solo la columna label.
    labels = ds_hemg["label"]
    ...
    # =====================================================
    # GUARDAR EN BASE DE DATOS (tabla model_artifacts)
    # =====================================================
    print("\n" + "=" * 60)
    ...
    POSTGRES_USER = os.getenv('POSTGRES_USER', 'fraudapp')
    POSTGRES_PASSWORD = os.getenv('POSTGRES_PASSWORD', 'fraudpass')
    POSTGRES_DB = os.getenv('POSTGRES_DB', 'frauddb')
    POSTGRES_HOST = os.getenv('POSTGRES_HOST', 'db')
    POSTGRES_PORT = os.getenv('POSTGRES_PORT', '5432')
    DATABASE_URL = f'postgresql://{POSTGRES_USER}:{POSTGRES_PASSWORD}@{POSTGRES_HOST}:{POSTGRES_PORT}/{POSTGRES_DB}'
    
    # Serializar el modelo en formato PyTorch (como espera clip_detector.py)
    buffer = BytesIO()
    torch.save({
    ...
    # Insertar en model_artifacts (la tabla que usa clip_detector.py)
    conn = psycopg2.connect(DATABASE_URL)
    cur = conn.cursor()
    
    cur.execute("""
        INSERT INTO model_artifacts (id, name, version, artifact, metrics, created_at)
        VALUES (%s, %s, %s, %s, %s, NOW())
    """, (artifact_id, "clip_head_global", version, psycopg2.Binary(artifact_bytes), metrics_json))
