#!/usr/bin/env python3
"""
Script para probar la precisión del detector con 100 imágenes AI y 100 reales.
"""
import sys
import os
...
def main():
    ...
    print("=" * 70)
    print("TEST DE PRECISIÓN: 100 imágenes AI + 100 imágenes reales")
    print("=" * 70)
    print()
    ...
    print("  Dataset: Hemg/AI-Generated-vs-Real-Images-Datasets")
    ...
    for item in ds:
        if item['label'] == 0 and len(ai_images) < 100:  # 0 = AI
            ai_images.append(item['image'])
        elif item['label'] == 1 and len(real_images) < 100:  # 1 = Real
            real_images.append(item['image'])
        
        if len(ai_images) >= 100 and len(real_images) >= 100:
            break
    
    print(f"✓ Recolectadas {len(ai_images)} imágenes AI")
    print(f"✓ Recolectadas {len(real_images)} imágenes reales")
    print()
    
    # Test de imágenes AI
    print("-" * 70)
    print("FASE 1: Analizando 100 imágenes generadas por IA")
    print("-" * 70)
    ...
    print("-" * 70)
    print("FASE 2: Analizando 100 imágenes reales")
    print("-" * 70)
    ...
    print("IMÁGENES AI (100 imágenes):")
    ...
    recall = ai_correct / len(ai_results)
